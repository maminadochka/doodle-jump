...
bool detectJump(int playerX, int playerY, sf::Vector2u platformPosition, sf::Texture platformTexture, int dy) {
	if ((playerX + PLAYER_RIGHT_BOUNDING_BOX > platformPosition.x) &&
		(playerX + PLAYER_LEFT_BOUNDING_BOX < platformPosition.x + platformTexture.getSize().x)        
		// player's horizontal range can touch the platform
		&& (playerY + PLAYER_BOTTOM_BOUNDING_BOX > platformPosition.y) &&
		(playerY + PLAYER_BOTTOM_BOUNDING_BOX < platformPosition.y + platformTexture.getSize().y)  
		// player's vertical   range can touch the platform 
		&& (dy > 0)) {
		return true;
	}
	else {
...
bool detectBlackHoleHit(int playerX, int playerY, sf:: CircleShape blackHole) {
...
void gameOverScreen(int score, int best_score) {
	while (window.isOpen())
	{
		sf::Event event;
...
void gameScreen() {
	...
	for (int i = 0; i < WINDOW_WIDTH / platformTexture.getSize().x + 1; ++i){
		platformPosition[i].x = x(e);
		platformPosition[i].y = y(e);
	}
	for (int i = 0; i < WINDOW_WIDTH / blackHole.getGlobalBounds().width + 1; ++i) {
	...
	while (window.isOpen())
	{
		...
		for (size_t i = 0; i < WINDOW_WIDTH / platformTexture.getSize().x + 1; ++i)
		{
			if (detectBlackHoleHit(playerX, playerY, blackHole)) {
				...
				gameOverScreen(score, best_score);
			}
		}
		player.setPosition(playerX, playerY);
